...
- (IBAction)_verifyButtonAction:(id)sender {
    ...
    if ([codestr hasPrefix:_appealTextField.text]) {
        ...
        [kUSerD synchronize];
        [XTOOLS showAlertTitle:@"验证成功" message:@"已经去除广告" buttonTitles:@[@"确定"] completionHandler:^(NSInteger num) {
        ...
        if ([codestr hasSuffix:_appealTextField.text]) {
            ...
            [kUSerD synchronize];
            [XTOOLS showAlertTitle:@"验证成功" message:@"已经恢复加密功能" buttonTitles:@[@"确定"] completionHandler:^(NSInteger num) {
