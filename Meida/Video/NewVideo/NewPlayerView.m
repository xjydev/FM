...
- (void)awakeFromNib {
    [super awakeFromNib];
    self.rotateStatus = 8;//初始化一个值,8也是上下结构。
    [self setupViews];
...
- (void)appActivityNotification {
    ...
    [self rotateScreenWithStatus:_currentO];
}
...
- (void)layoutSubviews {
    ...
    if (self.rotateStatus == 1 || self.rotateStatus == 3) {//横屏
...
- (void)rotateButtonAction {
    ...
    [self rotateScreenWithStatus:self.rotateStatus+1];
...
//    }
    ...
    NSLog(@"rotastatus == %@ ==%@",@(status),@(orientation));
...
- (void)setRotateStatus:(NSInteger)rotateStatus {
