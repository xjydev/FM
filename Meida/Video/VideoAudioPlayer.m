...
- (void)updateModel {
    if ([XTOOLS timeStrToSecWithStr:_player.time.stringValue]<[XTOOLS timeStrToSecWithStr:_player.media.length.stringValue]-10) {

        [[XManageCoreData manageCoreData]saveRecordName:[_player.media.metaDictionary objectForKey:VLCMetaInformationTitle]  path:_player.currentPath record:[XTOOLS timeStrToSecWithStr:_player.time.stringValue ] totalTime:_player.media.length.intValue/1000 iconPath:nil];
    }
    else
    {//播完了
        [[XManageCoreData manageCoreData]saveRecordName:[_player.media.metaDictionary objectForKey:VLCMetaInformationTitle]  path:_player.currentPath record:1.0 totalTime:_player.media.length.intValue/1000 iconPath:nil];
...
- (void)setIndex:(NSInteger)index {
    if (self.mediaArray.count>0) {
        ...
        NSObject *object = self.mediaArray[_index];
        if ([object isKindOfClass:[NSString class]]) {
            self.currentPath = (NSString *)object;
        }
        else {
            self.currentPath = ((Record *)object).path;
        }
    }
    else
    {
...
- (void)setCurrentPath:(NSString *)currentPath {
    ...
    //如果是其他数据类型就转换过来。
    if ([currentPath isKindOfClass:[Record class]]) {
        Record *r = (Record *)currentPath;
        currentPath = r.path;
    }
    ...
    if ([_currentPath isEqualToString:currentPath]) {//如果相同就展厅
        ...
        //如果是高清影视就调低
        [_player.media parseWithOptions:VLCMediaParseLocal|VLCMediaFetchLocal];
        if (self.isVideo) {
            if (self.playModelType == XPlayModelTypeSingle) {
                [_player play];
            }
        ...
        if (![kFileM fileExistsAtPath:currentPath]) {//如果不能找到文件
            [XTOOLS showAlertTitle:@"文件丢失" message:@"找不到文件或者文件已被删除了，是否删除此条记录？" buttonTitles:@[@"取消",@"删除"] completionHandler:^(NSInteger num) {
                if (num == 1) {
                    [[XManageCoreData manageCoreData]deleteRecordPath:currentPath];
                }
            ...
            if (_player&&[NSURL fileURLWithPath:_currentPath] != _player.media.url) {
                ...
                NSLog(@"media == %@",_player.media.metaDictionary);
                //如果是高清影视就调低
                [_player.media parseWithOptions:VLCMediaParseLocal|VLCMediaFetchLocal];//VLCMediaParseNetwork|VLCMediaFetchNetwork
                if (!self.isVideo) {
