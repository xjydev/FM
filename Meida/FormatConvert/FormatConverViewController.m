...
+ (instancetype)allocFromStoryBoard {
    ...
    FormatConverViewController *VC = [mediaStory instantiateViewControllerWithIdentifier:@"FormatConverViewController"];
    return VC;
}
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    alertHeaderLabel.text = @"此功能只支持少数格式，您可尝试进行转换";
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:
            {
                ...
                filesList.fileType = FileTypemedia;
...
- (void)formartConvertBegan {
    ...
    if ([compatiblePresets containsObject:AVAssetExportPresetHighestQuality]) {
        ...
        if (![exportSession.supportedFileTypes containsObject:_formatDict.allValues.firstObject]) {
        ...
        if ([kFileM fileExistsAtPath:resultPath]) {
            [kFileM removeItemAtPath:resultPath error:nil];
        }
        exportSession.outputURL = [NSURL fileURLWithPath:resultPath];
        
        exportSession.outputFileType =_formatDict.allValues.firstObject;//可以配置多种输出文件格式
         ...
         
         {
             ...
             switch (exportSession.status) {
                 ...
                 case AVAssetExportSessionStatusCompleted:
                 {
                     ...
                     if ([kFileM fileExistsAtPath:resultPath]) {
                        self->_convertedPath = resultPath;
                         self->_mainArray = @[@[@"选择文件：",@"转换后格式：",@"转换后质量："],@[@"转换后文件："]];
                         dispatch_async(dispatch_get_main_queue(), ^{
                             self->_convertButton.selected = NO;
                             self->_convertButton.userInteractionEnabled = YES;
                             [self->_mainTableView reloadData];
                             [XTOOLS hiddenLoading];
